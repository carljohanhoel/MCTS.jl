"""
MCTS solver with NN priors and DPW
...
Fields:
    ...
    next_action::Any
        ...
        The next action is determined based on the MDP, the state, `s`, and the current `AZStateNode`, `snode`.
...
"""
mutable struct AZSolver <: AbstractMCTSSolver
    depth::Int
    exploration_constant::Float64
    n_iterations::Int
    max_time::Float64
...
"""
function AZSolver(;depth::Int=10,
...
mutable struct AZTree{S,A}
    ...
    function AZTree{S,A}(sz::Int=1000) where {S,A}
        sz = min(sz, 100_000)
...
Base.isempty(tree::AZTree) = isempty(tree.n) && isempty(tree.q)

immutable AZStateNode{S,A} <: AbstractStateNode
    tree::AZTree{S,A}
    index::Int
end

children(n::AZStateNode) = n.tree.children[n.index]
n_children(n::AZStateNode) = length(children(n))
isroot(n::AZStateNode) = n.index == 1

mutable struct AZPlanner{P<:Union{MDP,POMDP}, S, A, SE, NA, RNG} <: AbstractMCTSPlanner{P}
    solver::AZSolver
    mdp::P
    tree::Nullable{AZTree{S,A}}
    solved_estimate::SE
    next_action::NA
    rng::RNG
    training_phase::Bool
...
Base.srand(p::AZPlanner, seed) = srand(p.rng, seed)
